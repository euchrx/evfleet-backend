...
export class SystemResetService {
  ...
  async resetAllDatabase(jwtSecretToken: string) {
    ...
    if (!configuredSecret) {
      throw new InternalServerErrorException('JWT_SECRET nao configurado no servidor.');
    }

    if (String(jwtSecretToken || '').trim() !== configuredSecret) {
      throw new UnauthorizedException('Token JWT_SECRET invalido para executar reset.');
    }
    ...
    // Operational data reset only.
    const tireReadings = await safeDeleteMany(() => this.prisma.tireReading.deleteMany());
    const fuelRecords = await safeDeleteMany(() => this.prisma.fuelRecord.deleteMany());
    ...
    const costCenters = await safeDeleteMany(() => this.prisma.costCenter.deleteMany());
    ...
    return {
      message: 'Reset do banco concluido com sucesso.',
      preserved: ['users', 'branches', 'administration'],
      deleted: result,
    };
  }
}
